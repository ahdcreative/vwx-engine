...
REPO_ROOT = "https://raw.githubusercontent.com/ahdcreative/vwx-engine/main/data/"
...
# Managed List of official ICAO Incidents
ICAO: List[str] = []
...
def nullify(data: dict) -> dict:
    """ Nullify empty strings inside a Dictionary """
...
def format_coord(coord: str) -> float:
    """ Convert coordinates from text to numbers """
...
def get_icao(station: List[str]) -> Optional[str]:
    """ Find a station by ICAO Code and GPS """
...
def format_station(code: str, station: List[str]) -> dict:
    """ Converts source station files list in a dictionary. """
    try:
        elev_ft = float(station[6])
        elev_mt = round(elev_ft * 0.3048)
        elev_ft = round(elev_ft)
    except ValueError:
        elev_ft, elev_mt = None, None
...
def build_stations() -> Tuple[dict, dict]:
    """ Build station dict form source """
    stations, code_map = {}, {}
    data = csv.reader(_SOURCE["airports"].splitlines())
    next(data)  # Skip Header
    for station in data:
        code = get_icao(station)
        if code and station[2] in ACCEPTED_TYPES:
...
def add_runways(stations: dict, code_map: dict) -> dict:
    """ Add runway information to station if available. """
    ...
    # Sort the runways by longest length and missing nulls
...
def check_local_icaos() -> None:
    """ Load the local ICAO file if available. Not included in distro. """
...
def check_local_awos() -> None:
    """ Load the local AWOS file if avaiable. Not included in distro. """
...
def download_source_files() -> bool:
    """ return true if source files are updated successfully """
...
def update_station_info_date() -> None:
    """ Update package station meta date """
...
def save_station_data(stations: dict) -> None:
    """ Saves the stations to JSON Package data. """
    json.dump(stations, OUT.open("w", encoding="utf8"), sort_keys=True, indent=1, ensure_ascii=False)


def main() -> int:
    """ Build or Update the main stations JSON file. """
    LOG.info("Fetching")
    ...
    LOG.info("Cleaning")
    clean_source_files()
    LOG.info("Building. This may take few moments")
    load_codes()
    ...
    LOG.info("Updating stations date")
