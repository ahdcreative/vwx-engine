...
from vwx.parsing.sanitization.base import CombineItems
from vwx.static.core import CLOUD_LIST, CLOUD_TRANSLATIONS, WIND_UNITS
...
class SeparatedWindUnit(CombineItems):
    ...
    def can_handle(self, first: str, second: str) -> bool:
        ...
        if (
                ...
                and (
                first[:5].isdigit()
                or (first.startswith("VRB") and first[3:5].isdigit())
        )
        ):
            return True
        # 36010K T
...
class SeparatedCloudQualifier(CombineItems):
    ...
    def can_handle(self, first: str, second: str) -> bool:
        return (
                second in CLOUD_TRANSLATIONS
                and second not in CLOUD_LIST
                and len(first) >= 3
                and first[:3] in CLOUD_LIST
        )
...
class SeparatedTafTimePrefix(CombineItems):
    ...
    def can_handle(self, first: str, second: str) -> bool:
        return first in {"FM", "TL"} and (
                second.isdigit() or (second.endswith("Z") and second[:-1].isdigit())
        )
